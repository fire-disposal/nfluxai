...
import yaml
from model_clients import ApiTextEmbeddings, get_embedding_service_config
...
def create_vectorstore(chunks: List[SemanticChunk], config: Dict[str, Any]):
    ...
    embedding_cfg = get_embedding_service_config(config)
    print("\n🔄 初始化嵌入 API 服务...")
    print(f"   provider: {embedding_cfg.get('provider')}")
    print(f"   model: {embedding_cfg.get('model')}")
    embeddings = ApiTextEmbeddings(embedding_cfg)
    ...
    batch_size = int(embedding_cfg.get("batch_size", config.get("embedding_batch_size", 128)))
    total = len(documents)
    if total == 0:
